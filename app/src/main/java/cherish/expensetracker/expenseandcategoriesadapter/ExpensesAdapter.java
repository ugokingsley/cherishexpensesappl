package cherish.expensetracker.expenseandcategoriesadapter;
...
public class ExpensesAdapter<VH extends RecyclerView.ViewHolder> extends ExpensesRecyclerViewAdapter<ExpensesAdapter.BaseExpenseViewHolder> {
    ...
    public ExpensesAdapter(Context context, BaseViewHolder.RecyclerClickListener onRecyclerClickListener) {
        this.mExpensesList = ExpensesManager.getInstance().getExpensesList();
        this.onRecyclerClickListener = onRecyclerClickListener;
        this.colorExpense = CherishExpenseTracker.getContext().getResources().getColor(R.color.colorAccentRed);
        this.colorIncome = CherishExpenseTracker.getContext().getResources().getColor(R.color.colorAccentGreen);
        this.prefixExpense = CherishExpenseTracker.getContext().getResources().getString(R.string.exp_pref);
        this.prefixIncome = CherishExpenseTracker.getContext().getResources().getString(R.string.income_prefix);
        this.titleTransitionName = CherishExpenseTracker.getContext().getString(R.string.title_transition);
    }
    ...
    protected void setAnimation(BaseExpenseViewHolder holder, int position) {
        if (position > lastPosition) {
            Animation animation = AnimationUtils.loadAnimation(CherishExpenseTracker.getContext(), R.anim.push_left_in);
